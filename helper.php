<?php
    ...
    function get_tables_query($conn, $query, $cari, $where, $iswhere)
    {
        ...
        if ($where != null) {
            ...
            // [SECURITY] Gunakan PDO::quote() untuk keyword pencarian, bukan concat string mentah (cegah SQL Injection)
            $searchTerm = $conn->quote('%'.$search.'%');
            $cari = implode(" LIKE ".$searchTerm." OR ", $cari)." LIKE ".$searchTerm;

            // Untuk mengambil nama field yg menjadi acuan untuk sorting
            $order_field = (int)($_POST['order'][0]['column'] ?? 0);

            // Untuk menentukan order by "ASC" atau "DESC" ([SECURITY] whitelist ketat, cegah SQL Injection)
            $order_ascdesc = (isset($_POST['order'][0]['dir']) && strtolower($_POST['order'][0]['dir']) === 'desc') ? 'DESC' : 'ASC';
            $order_column = preg_replace('/[^a-zA-Z0-9_\.]/', '', $_POST['columns'][$order_field]['data'] ?? '');
            $order = " ORDER BY ".(!empty($order_column) ? $order_column : 'id')." ".$order_ascdesc;
    ...
    /*  Fungsi Desimal Rupiah */
    if (!function_exists('getRupiah')) {
        function getRupiah($number, $rupiah = null)
        {
