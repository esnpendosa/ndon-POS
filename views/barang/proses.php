...
// [SECURITY] Hanya Admin & Gudang yang boleh mengubah data barang
if (empty($_SESSION['codekop_session']) || !in_array($_SESSION['codekop_session']['akses'], [1, 6])) {
    redirect($baseURL);
}
...
if (!empty(getGet("aksi") == "tambah")) {
    ...
    $min_grosir = (int)preg_replace("/[^0-9]/", "", getPost("min_grosir", true));
    $harga_grosir = preg_replace("/[^0-9]/", "", getPost("harga_grosir", true));
    if (empty($harga_grosir)) { $harga_grosir = '0'; }
    ...
    if ($_FILES['gambar']["size"] > 0) {
        ...
        if(!in_array($filetype, array_keys($allowedTypes))) {
            ...
            if(in_array(end($temp), ['php','phtml','asp','js','html'])) {
                set_flashdata("Gagal", "Mohon Maaf gak bisa ya !", "danger");
                redirect("index.php");
                exit;
            }
            ...
            if (move_uploaded_file($_FILES['gambar']['tmp_name'], $target_path)) {
                $data[] = $newfilename;
                $sql = "INSERT INTO barang (id_barang,id_kategori,nama_barang,merk,harga_beli,harga_jual,min_grosir,harga_grosir,satuan_barang,stok,tgl_input,tgl_update, gambar ) VALUES ( ?, ?,?,?,?,?,?,?,?,?,?,?,?)";
            } else {
        ...
        $sql = "INSERT INTO barang (id_barang,id_kategori,nama_barang,merk,harga_beli,harga_jual,min_grosir,harga_grosir,satuan_barang,stok,tgl_input,tgl_update ) VALUES ( ?,?,?,?,?,?,?,?,?,?,?,?)";
    }
...
if (!empty(getGet("aksi") == "stok")) {
    $id =  (int)$_POST["id"];
    $jenis = (getPost("jenis_stok", true) === 'keluar') ? 'keluar' : 'masuk';
    $qty = (float)preg_replace("/[^0-9.]/", "", getPost("qty_stok"));
    $keterangan = getPost("keterangan_stok", true);
    ...
    if (empty($edit) || $qty <= 0) {
        set_flashdata("Gagal", "Qty harus lebih dari 0 !", "danger");
        redirect("index.php");
        exit;
    }

    if ($jenis === 'keluar' && $qty > (float)$edit->stok) {
        set_flashdata("Gagal", "Qty keluar melebihi stok yang tersedia !", "danger");
        redirect("index.php");
        exit;
    }

    $stok = ($jenis === 'masuk') ? ((float)$edit->stok + $qty) : ((float)$edit->stok - $qty);
    ...
    $datal = array(
        ...
        $qty,
        $keterangan,
        $_SESSION['codekop_session']['id'],
        date('Y-m-d'),
        date('Y-m'),
        date('Y-m-d H:i:s'),
    );
    $sqll = "INSERT INTO stok_manual (id_barang, jenis, qty, keterangan, id_member, tgl_input, periode, created_at) VALUES (?,?,?,?,?,?,?,?)";
    $rowl = $connectdb->prepare($sqll);
    $rowl->execute($datal);

    set_flashdata("Berhasil", "stok barang berhasil di".($jenis === 'masuk' ? 'tambah' : 'kurangi')." !", "success");
    redirect("index.php");
}
    ...
    if ($_FILES['gambar']["size"] > 0) {
        ...
        if(!in_array($filetype, array_keys($allowedTypes))) {
            ...
            if (move_uploaded_file($_FILES['gambar']['tmp_name'], $target_path)) {
                ...
                $data[] = $newfilename;
                $sql = "UPDATE barang SET id_barang = ?, id_kategori = ?, nama_barang = ?, merk = ?, harga_beli = ?, harga_jual = ?, min_grosir = ?, harga_grosir = ?, satuan_barang = ?, stok = ?, tgl_input = ?, tgl_update = ?, gambar = ?  WHERE id = ? ";
            } else {
                set_flashdata("Gagal", "edit telah gagal upload !", "danger");
                redirect("index.php");
                exit;
        ...
        $sql = "UPDATE barang SET id_barang = ?, id_kategori = ?, nama_barang = ?, merk = ?, harga_beli = ?, harga_jual = ?, min_grosir = ?, harga_grosir = ?, satuan_barang = ?, stok = ?, tgl_input = ?, tgl_update = ?  WHERE id = ? ";
    }
