<?php
    if(!empty(getGet("aksi") == "keranjang")) {
        ...
        if($edit->stok >= (int)$_POST["qty"])
        {
            ...
            if ($jml > 0) {
                ...
                $new_qty = (int)$item_lama->jumlah + ((int)$_POST["qty"] ?? 1);
                $harga_jual = (float)$edit->harga_jual;
                if (!empty($edit->min_grosir) && (int)$edit->min_grosir > 0 && $new_qty >= (int)$edit->min_grosir && (float)$edit->harga_grosir > 0) {
                    $harga_jual = (float)$edit->harga_grosir;
                }
                $sqlw = "UPDATE keranjang SET jumlah= ?, jual = ? WHERE id_barang = ? AND id_member = ?";
                $roww = $connectdb->prepare($sqlw);
                $roww->execute(array($new_qty, $harga_jual, $id, $_SESSION["codekop_session"]["id"]));
            }else{
                $qty = (int)$_POST["qty"] ?? 1;
                $harga_jual = (float)$edit->harga_jual;
                if (!empty($edit->min_grosir) && (int)$edit->min_grosir > 0 && $qty >= (int)$edit->min_grosir && (float)$edit->harga_grosir > 0) {
                    $harga_jual = (float)$edit->harga_grosir;
                }
                $data = [];
                $data[] =  $id;
    ...
    if(!empty(getGet("aksi") == "carikeranjang")) {
        ...
        if($edit->stok >= (int)$_GET["qty"])
        {
            ...
            if ($jml > 0) {
                $item_lama = $rowq->fetch(PDO::FETCH_OBJ);
                $new_qty = (int)$item_lama->jumlah + ((int)$_GET["qty"] ?? 1);
                $harga_jual = (float)$edit->harga_jual;
                if (!empty($edit->min_grosir) && (int)$edit->min_grosir > 0 && $new_qty >= (int)$edit->min_grosir && (float)$edit->harga_grosir > 0) {
                    $harga_jual = (float)$edit->harga_grosir;
                }
    ...
    if(!empty(getGet("aksi") == "editkeranjang")) {
        ...
        if($edit && $edit->stok >= $qty)
        {
            ...
            $data[] = $harga_jual;
            $data[] = (int)$_POST['id'];
            $sqlk = "UPDATE keranjang SET diskon = ?, jumlah = ?, jual = ? WHERE id = ? ";
