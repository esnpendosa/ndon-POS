...
<div class="table-responsive pt-1">
    <table class="table table-striped text-sm table-sm" id="example5">
        ...
        <tbody>
            <?php
                $no =1;
                $sql = "SELECT barang.id as idba, barang.id_barang as idb, barang.stok, barang.harga_jual, barang.min_grosir, barang.harga_grosir, barang.gambar, keranjang.* 
                        FROM keranjang  
                        LEFT JOIN barang 
                ...
                foreach($hasil as $r) {
                    if($r->stok < $r->jumlah) {
                        $sqlw = "UPDATE keranjang SET jumlah= ? WHERE id_barang = ? AND id_member = ?";
                        $roww = $connectdb->prepare($sqlw);
                        $roww->execute(array($r->stok, $r->id_barang, $_SESSION["codekop_session"]["id"]));
                        echo "<script>
                                Swal.fire({
                                    title: 'Failed !',
                                    html: 'jumlah melebih stok qty barang',
                                    icon: 'warning',
            ...
            <tr>
                ...
                <td><?=$r->nama_barang;?></td>
                <td>
                    <b><?= getRupiah($r->jual, 'Rp');?></b>
                    <?php if (!empty($r->min_grosir) && (int)$r->min_grosir > 0 && (float)$r->harga_grosir > 0): ?>
                        <?php if ((float)$r->jual == (float)$r->harga_grosir): ?>
                            <div><span class="badge badge-success"><i class="fas fa-tags mr-1"></i>Grosir</span></div>
                        <?php else: ?>
                            <div class="text-muted" style="font-size: 10px;">Grosir &ge; <?= $r->min_grosir; ?>: <?= getRupiah($r->harga_grosir, 'Rp'); ?></div>
                        <?php endif; ?>
                    <?php endif; ?>
                </td>
                ...
                <td>
                    <input type="number" class="form-control form-control-sm" name="qty" data-id="<?= $r->id;?>"
                        ...
                        style="width:65px !important;" value="<?=$r->jumlah;?>">
                </td>
                <td>
                    <input type="number" class="form-control form-control-sm" name="diskon" data-id="<?= $r->id;?>"
                        ...
                        style="width:95px !important;" value="<?=$r->diskon;?>">
                </td>
                <td><?= getRupiah((($r->jual * $r->jumlah) - $r->diskon) ?? 0,'Rp');?>
                </td>
                <td>
...
function ajaxKeranjang(id_barang, qty, diskon, id) {
    $.ajax({
        'type': 'POST',
        'url': '<?= $baseURL.'helper/transaksi/proses.php?aksi=edit_keranjang';?>',
        'data': {
            "id_barang": id_barang,
            "qty": qty,
            "diskon": diskon,
