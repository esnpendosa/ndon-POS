    ...
    <table class="table table-striped text-sm table-sm" id="example5">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><?= getRupiah(($r->beli * $r->jumlah) ?? 0,'Rp');?></td>
                <td>
...
function ajaxKeranjangJual(jual, tipe, id) {
    $.ajax({
        'type': 'POST',
        'url': '<?= $baseURL.'helper/transaksi_beli/proses.php?aksi=edit_jual';?>',
        'data': {
            'tipe': tipe,
            "jual": jual,
            "id": id
...
function ajaxKeranjang(id_barang, qty, beli, jual, id) {
    $.ajax({
        'type': 'POST',
        'url': '<?= $baseURL.'helper/transaksi_beli/proses.php?aksi=edit_keranjang';?>',
        'data': {
            "id_barang": id_barang,
            "qty": qty,
            "beli": beli,
